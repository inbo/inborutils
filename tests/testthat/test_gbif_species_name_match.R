...
testthat::test_that("check validity of parameter values", {
  ...
  expect_error(
    gbif_species_name_match(df3, gbif_terms = c("family")),
    paste(
      "These columns exist in colnames but not",
  ...
  expect_error(gbif_species_name_match(df, kingdomsss = "kingdom_col"),
    paste(
      "Only optional parameters of GBIF name search",
  ...
  expect_warning(
    ...
    paste0(
      "Column with names \'family\' is also one of the",
  ...
  expect_warning(
    gbif_species_name_match(df, gbif_terms = c(
      "class",
      "classKey",
      "genus",
      "genusKey"
...
testthat::test_that("Test output", {
  ...
  out2 <- gbif_species_name_match(df2, gbif_terms = c(
    "species",
    "kingdom"
  ))
